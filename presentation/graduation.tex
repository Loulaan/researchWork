...
%		\bigskip
%		{\color{blue} Постановка численного эксперимента:}
		
%		Зафиксируем остальные параметры --- $ N=700 $, $ Q = 301 $, $ L=60 $, $ B=T=100 $, $ \sigma^2 = 0.25 $, $ \omega_1 = 0.1 $, $ \omega_2 = 0.2 $, $ C_1 = 1 $, $ C_2 = 2 $, $ \phi_1=0 $, $ \phi_2 = \frac{\pi}{2} $.
		
%		Промоделируем $ 200 $ реализаций шума $ \epsilon $ и на каждой итерации берем $ \gamma^* $ --- максимум функции обнаружения на промежутке до момента $ Q $. Начиная с $ Q $ определять момент преодоления $ n_{overcome} $ значения $ \gamma^* $, количество этих преодолений $\#n_{overcome}$ и оценивать скорость роста функций в точках $ [Q, Q+10, Q+20, Q+30] $.
	...
	\begin{frame}
		\frametitle{Сравнение функций обнаружения}
		\begin{figure}[!hhh]
			\center{\includegraphics[width=\linewidth]{imgs/detectionNoisedTests}}
...
%	\begin{frame}
%		\frametitle{Сравнение функций обнаружения: результаты}
%		\begin{table}[!hhh]
%			\center
%			\small
%			\caption{Средние значения характеристик функций обнаружения по итерациям и %всем типам неоднородности.}
...
%	\end{frame}
	...
	\begin{frame}
		...
		\bigskip
		{\color{blue} Постановка задачи:}
		Аналитически упростить индекс неоднородности $ g(F^{(1)}; F^{(2)}) $ для строковой функции обнаружения $ d_{n-1}^{(r)} $, чтобы явно увидеть, как частоты ряда до и после момента $ Q $ влияют на его значения.
	...
	\begin{frame}
		\frametitle{Система обнаружения момента возмущения: пример работы}
			\begin{figure}[!hhh]
				\center{\includegraphics[width=1\linewidth]{imgs/example_system_work}}
	...
	\begin{frame}
		...
		\begin{figure}[!hhh]
			\center{\includegraphics[width=0.9\linewidth]{imgs/row_diff_small_B.png}}
			\caption{Функция $ d_n $. Реализации шума, $ B=90 $.}
			\label{pic:row_diff_small_B}
		\end{figure}
		
		\begin{figure}[!hhh]
			\center{\includegraphics[width=0.9\linewidth]{imgs/row_diff_big_B.png}}
...
%	\end{frame}	
	...
	\begin{frame}
		\frametitle{Проблемы}
		\begin{figure}[!hhh]
			\center{\includegraphics[width=0.8\linewidth]{imgs/example_system_work}}
			\caption{Система. Пример работы.}
		\end{figure}
		\begin{figure}[!hhh]
			\center{\includegraphics[width=0.8\linewidth]{imgs/diff_moment_growth_most_unique.png}}
