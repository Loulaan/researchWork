...
%	\end{frame}
	...
	\begin{frame}
		...
		\bigskip
		{\color{blue} Постановка задачи:}
		Аналитически упростить индекс неоднородности $ g(F^{(1)}; F^{(2)}) $ для строковой функции обнаружения $ d_{n-1}^{(r)} $, чтобы получить в явном виде зависимость от частот до и после разладки.
