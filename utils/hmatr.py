...
class Hmatr:
    ...
    def update_hmatr(self, newValueOfSeries):
        ...
        self.th = np.vstack(
            [self.th, np.array(self.f[self.K - 1:self.N]).reshape(1, -1)])  # Добавляем в ганкелеву матрицу новую строку
        self.cth2_cumsum = np.r_[self.cth2_cumsum, self.cth2_cumsum[-1] + np.sum(
            self.th[-1] ** 2)]  # Добавляем новый элемент в кумулятивные суммы
        row = self.getNewRow()
        col = self.getNewCol()
