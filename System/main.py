...
def main():
    N = 700  # Длина ряда
    w1 = 1 / 10  # Начальная частота
    delta_min = 1 / 15  # Минимальная разница в частотах для обнаружения неоднородности
    k = 30  # Кол-во точек, за которые нужно обнаружить разладку
    w2 = 1/8
    Q = 301
    ...
    system = System()
    Q_hat = system(F=original_series, k=k, w1=w1, delta_min=delta_min, sigma_sq=0.25)
    print(Q_hat)
